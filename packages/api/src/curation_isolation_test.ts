...
function readSourceFiles(): { path: string; content: string }[] {
  ...
  for (const entry of Deno.readDirSync(SRC_DIR)) {
    if (!entry.isFile || !entry.name.endsWith(".ts")) continue;
    if (entry.name.endsWith("_test.ts")) continue;
    if (entry.name === "curation_isolation_test.ts") continue;
    files.push({ path: entry.name, content: Deno.readTextFileSync(SRC_DIR + entry.name) });
  }
  return files;
}
...
Deno.test("isolation: curation.ts/blocklist loaders only ever read the config files via static import, never construct a dynamic path", () => {
  const curation = Deno.readTextFileSync(SRC_DIR + "curation.ts");
